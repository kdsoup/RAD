...
public static class CountSketch
{
    ...
    public static BigInteger AdvancedFourUniversalHashFunction(BigInteger x, BigInteger[] a)
    {
        int b = 89;
        BigInteger y;
        BigInteger p = BigInteger.Pow(2, b) - 1;
    ...
    public static Tuple<BigInteger, BigInteger> AdvancedHashFunction(int t, BigInteger x, BigInteger[] a)
    {
        int b = 89;
        ulong m = (ulong)Math.Pow(2, t);
        BigInteger gx = AdvancedFourUniversalHashFunction(x, a);
        BigInteger hx = gx & (m - 1);
        BigInteger bx = gx >> (b - 1);
        BigInteger sx = 1 - 2 * bx;
    ...
    public static BigInteger[] AdvancedCountSketch(IEnumerable<Tuple<ulong, int>> stream, BigInteger[] a, int k)
    {
        int n = stream.Count();
        BigInteger[] C = new BigInteger[k];
        for (int j = 0; j < n; j++)
        {
            (ulong x, int delta) = stream.ElementAt(j);
            (var hx, var sx) = AdvancedHashFunction(t, x, a);
            C[(int)(hx % k)] += delta * sx;
        }
        return C;
    }
    ...
    public static BigInteger AdvancedQuery(ulong x, BigInteger[] CountSketch, BigInteger[] a)
    {
        (var hx, var sx) = AdvancedHashFunction(t, x, a);
        return sx * CountSketch[(int)hx];
    }
    ...
    {
        return countSketch.Aggregate(BigInteger.Zero, (acc, x) => acc + BigInteger.Pow(x, 2));
    }
